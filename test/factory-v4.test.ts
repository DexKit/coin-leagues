...
describe("CoinLeagueFactoryV4", function () {
  /*- let factory;
   let settings;
  ...
  it("Should create a full Game from factory, abort and anyone able to claim", async function () {
    const Settings = await ethers.getContractFactory("CoinLeagueSettingsETH");
    const CoinLeagueFactory = await ethers.getContractFactory("CoinLeagueV3Factory");
    const settings = await Settings.deploy();
    await settings.deployed();
    console.log(settings.address);
    const [owner, ...rest] = await ethers.getSigners();
    ...
    // 0.1 ETH
    const amount = BigNumber.from('10').pow('17');
    const nativeCoin = '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';
    const totalAmount = BigNumber.from(players_playing).mul(amount);
    const num_coins = `${allCoins.length}`;
    const blockNumber = await ethers.provider.getBlockNumber();
    const abortDate = (await ethers.provider.getBlock(blockNumber)).timestamp + 60;
    const startDate = (await ethers.provider.getBlock(blockNumber)).timestamp - 1;
    await factory.createGame(num_players, duration, amount, num_coins, abortDate, startDate, 0, nativeCoin);

    allCoins.pop();
    let joinedGame = await factory.connect(owner).joinGameWithCaptainCoin(allCoins, allCoins[0], owner.address, 500000, 0, { value: amount });
    expect(factory.connect(owner).joinGameWithCaptainCoin(allCoins, allCoins[0], owner.address, 500000, 0, { value: amount })).to.be.revertedWith('You Already joined');
    await joinedGame.wait();
    // We create the other 8 players now
    for (let index = 0; index < 8; index++) {
      const element = rest[index];
      const joinedGame = await factory.connect(element).joinGameWithCaptainCoin(allCoins, allCoins[0], element.address, 500000, 0, { value: amount, gasLimit: 25000000 });
      await joinedGame.wait();
    }
    expect((await factory.game(0)).total_amount_collected).to.equal(totalAmount.toString());
    expect(await factory.totalPlayers(0)).to.equal(players_playing);
    expect((await factory.game(0)).started).to.equal(false);
    const blocksToAdvance = 5 * 60
    for (let index = 0; index < blocksToAdvance; index++) {
      await network.provider.send('evm_mine');
    ...
    const abortGame = await factory.connect(rest[0]).abortGame(0);
    await abortGame.wait();
    expect((await factory.game(0)).aborted).to.equal(true);
    const balance = await ethers.provider.getBalance(rest[1].address) as BigNumber;

    const withdrawGame = await factory.connect(owner).withdraw(rest[1].address, 0);

...
contract BattleNFTFactoryToken is Ownable {
    ...
    function joinGame(
        ...
        if (game.coin_to_play == NativeCoin) {
            require(game.entry == msg.value, "Sent exact value");
        } else {
            IERC20(game.coin_to_play).safeTransferFrom(
                msg.sender,
                address(this),
                game.entry
            );
    ...
    function claim(uint256 id) external {
        ...
        if (game.coin_to_play == NativeCoin) {
            ...
            IERC20(game.coin_to_play).safeTransfer(game.winner, amountWinner);
            IERC20(game.coin_to_play).safeTransfer(HOUSE, amountHouse);
        }
    ...
    function abortGameAndWithdraw(uint256 id) external {
        ...
        if (game.coin_to_play == NativeCoin) {
            ...
            IERC20(game.coin_to_play).safeTransfer(game.player1, game.entry);
        }

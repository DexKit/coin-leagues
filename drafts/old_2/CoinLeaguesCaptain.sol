...
contract CoinLeaguesCaptain {
     ...
     */
    function joinGameWithCaptainCoin(address[] memory coin_feeds, address captain_coin, uint256 champion_id) external payable {
        require(players.length < game.num_players, "Game already full");
        require(game.num_coins == coin_feeds.length + 1, "Exceed supported coins");
        require(
        ...
        if(champion_id != 500000){
            require(CHAMPIONS.ownerOf(champion_id) == msg.sender, "You need to own these champions");
        }
        require(ICoinsLeagueSettings(game.settings).isChainLinkFeed(captain_coin), "Feed not supported");
    ...
    function startGame() external {
        ...
        for (uint256 index = 0; index < players.length; index++) {
            ...
            // updated captain_coin
            Coin storage coin = coins[pl.captain_coin];
            coin.start_price = getPriceFeed(pl.captain_coin);
        }
    ...
    function endGame() external {
        ...
        for (uint256 index = 0; index < players.length; index++) {
            ...
            for (uint256 ind = 0; ind < pl.coin_feeds.length; ind++) {
             ...
             Coin storage coin = coins[pl.captain_coin];
             coin.end_price = getPriceFeed(pl.captain_coin);
             coin.score =
                    ((coin.end_price - coin.start_price)*1000) /
                    coin.end_price;
        }
        ...
        for(uint256 index = 0; index < players.length; index++) {
            ...
            Coin memory coin = coins[pl.captain_coin]; 
            // We only apply captain multiplier if effect is possitiv
            if()    
            if(pl.champion_id != 500000 && CHAMPIONS.ownerOf(champion_id) == pl.player_address ){
                 pl.score = pl.score + coin.score 
                   * (ICoinsLeagueSettings(game.settings).getHoldingMultiplier()/1000) 
                   // rarity goes from 1 to 7
                   * (ICoinsLeagueSettings(game.settings).getChampionsMultiplier(CHAMPIONS.rarity(champion_id) - 1)/1000  ;
            }else{
                pl.score = pl.score + coin.score * ICoinsLeagueSettings(game.settings).getHoldingMultiplier()/1000;
            }
